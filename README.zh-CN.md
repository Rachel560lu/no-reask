...
No Re-Ask 不等于跳过安全审批，也不会压过宿主或工具的权限门槛。它判断的是 Agent 是否应该在聊天中重复询问同一个问题；周围系统实际执行的权限限制仍然完整有效。

如果下一步具有破坏性或不可逆、超出已请求范围，或者确实缺少关键选择、授权或安全事实，Agent 必须停下来，只提出一个聚焦的实质性问题。
